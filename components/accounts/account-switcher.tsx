...
export function AccountSwitcher() {
  ...
  const [registerForm, setRegisterForm] = useState<{
    ...
    staffCode: string;
    avatar: (typeof AVATAR_CHOICES)[number];
  }>({
    name: '',
    username: '',
    password: '',
  ...
  const handleSignIn = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error instanceof TypeError) {
        setSignInError('Unable to reach the server. Please try again.');
      } else {
              ...
              <div>
                ...
                <div className="mt-2 grid grid-cols-4 gap-2">
                  {AVATAR_CHOICES.map((emoji) => {
                    const isSelected = registerForm.avatar === emoji;
                    return (
                      <button
                        key={emoji}
                        type="button"
                        onClick={() => setRegisterForm((prev) => ({ ...prev, avatar: emoji }))}
                        className={`flex h-10 w-full items-center justify-center rounded-lg border text-xl transition ${
                          isSelected
                            ? 'border-primary bg-primary/10 text-primary'
                            : 'border-border bg-muted/40 hover:border-primary/60 hover:text-primary'
                        }`}
