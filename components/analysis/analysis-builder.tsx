...
const validateSignificanceConfig = (
  ...
  if (!Number.isFinite(config.significanceLevel) || config.significanceLevel < 0) {
    return { valid: false, error: 'Enter a valid significance level (0 or higher).' } as const;
  }
...
export function AnalysisBuilder() {
  ...
  const handleDistributionBucketsChange = (value: string) => {
    ...
    setForm((previous) => {
      ...
      const buckets = Number.isNaN(parsed) ? base.buckets : Math.max(1, parsed);
  ...
  const handleSignificanceLevelChange = (value: string) => {
    ...
    setForm((previous) => {
      ...
      return {
        ...
        config: {
          ...base,
          significanceLevel: Number.isNaN(parsed) ? base.significanceLevel : Math.max(0, parsed)
        },
        error: null
      };
    });
              ...
              {steps.length > 0 ? (
                ...
                {form.methodId === 'one-sided-distance' && form.column && oneSidedFormConfig && (
                  <div className="space-y-4 rounded-xl border border-white/10 bg-background/80 p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(['linear', 'quadratic'] as const).map((mode) => (
                          <button
                            key={mode}
                            type="button"
                          ...
                          >
                            {mode === 'linear' ? 'Linear' : 'x² ramp'}
                          </button>
                        ))}
                      </div>
                      <p className="text-xs text-muted-foreground">
                        Linear scaling increases scores evenly; the x² ramp rewards the furthest rows more aggressively.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {form.methodId === 'zero-to-one-scaling' && form.column && zeroToOneFormConfig && (
                  <div className="space-y-4 rounded-xl border border-white/10 bg-background/80 p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(['linear', 'exponential'] as const).map((mode) => (
                        <button
                          key={mode}
                          type="button"
                        ...
                        >
                          {mode === 'linear' ? 'Linear' : 'Exponential'}
                        </button>
                      ))}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      The exponential option squares the distance so rows near the maximum earn higher scores faster.
                    </p>
                  </div>
                )}
                ...
                {form.methodId === 'distribution-density' && form.column && distributionFormConfig && (
                  <div className="space-y-4 rounded-xl border border-white/10 bg-background/80 p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Logarithmic scoring emphasises the preferred buckets while keeping others closer to 0.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {form.methodId === 'significance-flag' && form.column && significanceFormConfig && (
                  <div className="space-y-4 rounded-xl border border-white/10 bg-background/80 p-4">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        Flag rows whose value is at or below the specified significance level.
                      </p>
                    </div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Non-significant rows automatically receive the opposite score.
                    </p>
                  </div>
                )}
              </div>
